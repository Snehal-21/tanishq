...
function adminLogin(event) {
    ...
    if (email && password) {
        let flag = false;
        let adminStorage = JSON.parse(localStorage.getItem("adminUsers"));
        for (let i = 0; i < adminStorage.length; i++) {
            if (email == adminStorage[i].email && password == adminStorage[i].password) {
                flag = true;
                adminLogin = adminStorage[i];
            }
        }
        if (flag==true) {
